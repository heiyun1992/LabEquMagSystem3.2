				...
				<div class="main_container" id="dashboard_page1">
						 <table class="table table-striped">
							...
							<c:forEach var="list" items="${loanEquipmentLists}">
								<tr>
									<td>${list.equipment.name}</td>
									<td>${list.equipment.type}</td>
									<td>${list.teacher.name}</td>
									<td>${list.admin.name}</td>
									<td>${list.loan_date}</td>
									<td align="center" style="HEIGHT: 22px" align="center" width="4%">							
								<td type="button" class="btn btn-primary"
					...
					<ul>
						<li><a href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=1">首页</a></li>
						<c:choose>
							<c:when test="${page.pageNow - 1 > 0}">
								<li><a href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=${page.pageNow - 1}">上一页</a></li>
							</c:when>
							<c:when test="${page.pageNow - 1 <= 0}">
								<li><a href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=1">上一页</a></li>
							</c:when>
						</c:choose>

						<c:forEach var="item" varStatus="status" begin="1"
							end="${page.pageNow-1}">
							<li><a href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=${status.index}">${status.index}</a></li>
						</c:forEach>
						<li class="active"><a
							href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=${page.pageNow}">${page.pageNow}</a></li>
						...
						<c:choose>
							<c:when test="${page.totalPageCount==0}">
								<li><a href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=${page.pageNow}">下一页</a></li>
							</c:when>
							<c:when test="${page.pageNow + 1 < page.totalPageCount}">
								<li><a href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=${page.pageNow + 1}">下一页</a></li>
							</c:when>
							<c:when test="${page.pageNow + 1 >= page.totalPageCount}">
								<li><a href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=${page.totalPageCount}">下一页</a></li>
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${page.totalPageCount==0}">
								<li><a href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=${page.pageNow}">尾页</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="${pageScope.basePath }loan/getAllLoanEquipment?pageNow=${page.totalPageCount}">尾页</a></li>
							</c:otherwise>
						</c:choose>
					</ul>
				</div>				
