					...
					<div class="span12">
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>教师列表</div>
								...
								<table class="table table-striped table-hover table-bordered" id="sample_editable_1">
									...
									<c:forEach var="list" items="${teacherList}">
										<tr class="">
											...
											<td><a class="btn mini" href="${pageScope.basePath }teacher/updTeacher?updId=${list.id}">${list.name}</a></td>
											<td>${list.sex}</td>
											<td>${list.teacher_num}</td>
											<td>${list.phone_num}</td>
											<td>${list.email}</td>
											
											<td>											
												<a class="btn mini blue" href="${pageScope.basePath }teacher/updTeacher?updId=${list.id}" > 编辑</a>
												<button  class="btn mini blue"
								 onclick="if(window.confirm('确定要删除[${list.name}]吗?')==true){window.location.href='${pageScope.basePath }teacher/delTeacher?delId='+${list.id}}">
								 删除</button> 
											</td>
										</tr>	
										</c:forEach>				
